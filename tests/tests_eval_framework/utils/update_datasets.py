...
Downloads every task's dataset at its pinned revision. Already-cached revisions are cheap
no-ops, so the same command serves both a full build and an incremental top-up after a
pin change.
"""
