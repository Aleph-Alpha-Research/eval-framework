...
class TestNoSubject:
    def test_no_selection_yields_the_single_unnamed_slice(self) -> None:
        # Given a task with no subjects, when parsing no selection, then one slice with no config to load
        assert NoSubject().select([]) == (Subject(load_key=None, label="no_subject"),)

    def test_wildcard_yields_the_single_slice(self) -> None:
        # "*" means "all", which for a subjectless task is the one slice
        assert NoSubject().select(["*"]) == (Subject(load_key=None, label="no_subject"),)

    def test_specific_selector_is_rejected(self) -> None:
        # A no-subject task cannot select a named subject
        with pytest.raises(ValueError, match="no subjects"):
            NoSubject().select(["deu"])


class TestListOfSubjects:
    def test_no_selection_yields_all_in_declared_order(self) -> None:
        assert [s.label for s in ListOfSubjects(["a", "b", "c"]).select([])] == ["a", "b", "c"]

    def test_each_name_is_both_load_key_and_label(self) -> None:
        (subject,) = ListOfSubjects(["a"]).select([])
        assert subject == Subject(load_key="a", label="a")

    def test_wildcard_yields_all(self) -> None:
        assert [s.label for s in ListOfSubjects(["a", "b"]).select(["*"])] == ["a", "b"]

    def test_selection_follows_declared_order_and_dedupes(self) -> None:
